...
type Service interface{
	...
	//登录
   Login(name,pwd,roleId,rolaName string)(string,error)
}
...
func (s ArithmeticService) Login(name,pwd,roleId,rolaName string) (string,error){
	//数据库请求
	if name == "mike" && pwd == "123456" {
		//生成TOKEN
		token, err := Sign(name, pwd,roleId,rolaName)
		return token, err
	}
